...
function [x_est, P_est, indices] = E9(odo, zind, z, V, W, range, fov, x0, P0)
    % RANGE-ONLY SLAM. Need to get 2x2 from W.
    W_r = [W, 0; 0, fov(2)^2];
    ...
    for t = 1:(T-1)
        ...
        if ~isempty(zind{1,t})
            ...
            for i_lm = 1:num_landmarks_detected
                ...
                % RANGE-ONLY SLAM: CANNOT USE beta (already removed from z).
                r = z{1,t}(1,i_lm);
                % use mean fov as our bearing "measurement".
                beta = 0.5 * fov(1) + 0.5 * fov(2);
                rb = [r; beta]; 
    ...
    %                        x_predicted(2) + r*sin(x_predicted(3)+beta)];
                    ...
                    nu = rb - z_est - [w_r; w_b];
                    % compute kalman gain.
                    S = H_x * P_predicted * H_x' + H_w * W_r * H_w';
                    ...
                    % update covariance.
                    P_predicted = Y_z * [P_predicted, zeros(n,2); zeros(2,n), W_r] * Y_z';
                end
