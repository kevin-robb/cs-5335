...
T = 20; % # of timesteps.
% x = ones(3*T,1); % 3Tx1. format: [x1,x2,..,xT,y1,y2,...,yT,v1,...,vT].
x = [zeros(T,1)+x_start(1);zeros(T,1)+x_start(2);zeros(T,1)+x_start(3)];
% need to make matrices Aeq and Beq s.t. Aeq * u = Beq for valid trajectories.
% i.e., start and end should match x_start, x_goal. initial variance too.
Aeq = zeros(5,3*T); % select out the points we want to check.
Aeq(1,1) = 1; Aeq(2,T+1) = 1; Aeq(3,2*T+1) = 1;
Aeq(4,T) = 1; Aeq(5,2*T) = 1; %Aeq(6,3*T) = 1;
Beq = [x_start; x_goal(1:2)]; % 5x1.
...
% add a row to ensure the final variance <= specified val.
A = [A; zeros(1,3*T-1), 1];
B = [B; x_goal(3)];
...
nl_con_fn = @(x) nl_con_c(x, max_control, process_variance, variance_epsilon);
...
function cost = cost_fn_c(x)
    ...
    % add the variance to the cost.
%     cost = cost + sum(x(2*T+1:3*T));
end

%%%%%%%%%%%%%%%%%% NONLINEAR CONSTRAINT FUNCTION %%%%%%%%%%%%%%%%%%%%%%%%%
% OPT-c light-dark domain.
% @param x = 3Tx1 matrix of positions + variances.
function [c,ceq] = nl_con_c(x, max_control, process_variance, epsilon)
    ...
    for i = 1:T-1
        ...
        v_next = v(x(i), x(2*T+i));
        % however, do not allow to it shrink below epsilon.
        if v_next < epsilon
            v_next = epsilon;
        end
        ceq(i) = v_next - x(2*T+i+1);
    end
end
